...
One place that opens a run, applies runtime setting overrides, and drives the
stages — used by the CLI (`run-all`), the scheduler (per profile), and the web
"run now". Keeps overrides and run-tracking consistent across all three.
"""
...
def _run(session, run, specs) -> dict:
    ...
    collected = run_collect(session, run, specs)
